...
<script
	src="${context}/ui/resources/js/jquery-ddSlick/jquery.ddslick.min.js"></script>
...
<script>
	$(function() {
		...
		$(".btn-create-contract-review").click(
				function() {
					var context = $("#webContext").val();
					var url = context + "/contractReview/create";
					var taskId = getTaskId($(this));
					window.location.href = context + "/contractReview/create?taskId="
							+ taskId;
				});
		$(".btn-create-work-task").click(
				function() {
					var context = $("#webContext").val();
					var url = context + "/workTask/create";
					var taskId = getTaskId($(this));
					window.location.href = context + "/workTask/create?taskId="
							+ taskId;
				});
		$(".btn-assign-business-employee")
				.click(
						function() {
							var context = $("#webContext").val();
											...
											function(result) {
												if (result.result == "success") {
													...
													for (var i = 0; i < result.employeeList.length; i++) {
														...
														ddDataItem.imageSrc = context
																+ result.employeeList[i].employeeAvatar;
		...
		$(".btn-create-consumer-resource").click(
				function() {
					var context = $("#webContext").val();
					var url = context + "/consumerResource/create";
					var taskId = getTaskId($(this));
					window.location.href = context
							+ "/consumerResource/create?taskId=" + taskId;
				});
		$(".btn-create-field-survey").click(
				function() {
					var context = $("#webContext").val();
					var url = context + "/fieldSurvey/create";
					var taskId = getTaskId($(this));
					window.location.href = context
							+ "/fieldSurvey/create?taskId=" + taskId;
				});
	});
</script>
</head>
<body>
									...
									<table class="table">
										...
										<tbody>
											<tr>
												...
												<td><c:out
														value="${taskItem.projectInfo.projectStatus}" /></td>
												<td><c:out value="${taskItem.taskType}" /></td>
												<c:choose>
													<c:when test="${taskItem.taskType == '创建合同评审记录'}">
														<td><button
																class="btn btn-primary btn-create-contract-review">创建</button></td>
													</c:when>
													<c:when test="${taskItem.taskType == '合同评审记录签字'}">
														<td><button
																class="btn btn-primary btn-sign-contract-review">签字</button></td>
													</c:when>
													<c:when test="${taskItem.taskType == '创建工作任务单'}">
														<td><button
																class="btn btn-primary btn-create-work-task">创建</button></td>
													</c:when>
													<c:when test="${taskItem.taskType == '指定项目负责人'}">
														<td><button
																class="btn btn-primary btn-assign-business-employee">指定</button></td>
													</c:when>
													<c:when test="${taskItem.taskType == '创建客户资料登记单'}">
														<td><button
																class="btn btn-primary btn-create-consumer-resource">创建</button></td>
													</c:when>
													<c:when test="${taskItem.taskType == '创建现场调查记录'}">
														<td><button
																class="btn btn-primary btn-create-field-survey">创建</button></td>
													</c:when>
												</c:choose>
											</tr>
										</tbody>
	...
	<div class="modal fade" id="assignBusinessEmployeeModal" tabindex="-1"
		role="dialog" aria-labelledby="assignBusinessEmployeeModalLabel"
		aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button"
						class="btn btn-primary btn-assign-business-employee-confirm">确定</button>
				</div>
			</div>
		</div>
	</div>
